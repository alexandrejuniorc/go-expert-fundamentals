...
func main() {
	...
	//ctx, cancel := context.WithDeadline(ctx, time.Now().Add(time.Second)) // o contexto cancela a requisição após 1 segundo

	ctx := context.Background()                               // cria um contexto vazio
	ctx, cancel := context.WithTimeout(ctx, time.Microsecond) // o contexto cancela a requisição após 1 segundo
